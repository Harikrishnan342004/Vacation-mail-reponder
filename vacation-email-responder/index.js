...
app.get("/", async (req, res) => {
  ...
  const auth = await authenticate({
    keyfilePath: path.join(__dirname, "crd.json"),
    scopes: SCOPES,
  });
  ...
  //  this function generating the label ID        // Label Creating function
      ...
      if (messages && messages.length > 0) {
        for (const message of messages) {
          ...
          if (!hasReplied) {
            ...
            };   //This is out message sending function
...
});    //server port running on 8080 at nodemon